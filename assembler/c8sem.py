...
if not (2 <= len(sys.argv) <= 3):
    print("\n    Assembler Usage:")
    print("    \"c8sem.py SourceFilePath [RomFilePath]\"\n")
    print("    SourceFilePath: Chip-8 source file, usually a .src file extension")
    ...
    print("                    a .c8 extension\n")
    print("    e.g \"c8sem.py source/test.src roms/test_rom.c8\"\n")
else:
    ...
    if len(sys.argv) == 3:    # Out file name was provided
        ...
        slash_pos = filename_in.rindex('/')
        if filename_in.find('.',slash_pos + 1) > 0:
            filename_out = filename_in[:filename_in.rfind('.')] + '.c8'
        else:
            filename_out = filename_in + '.c8'
